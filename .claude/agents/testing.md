...
11. **Page presence and absence assertions belong to `AppDriver`, not feature drivers.** Use `AppDriver.assertIsOnPage(PageName.<page>)` / `assertPageExists(PageName.<page>)` and `AppDriver.assertPageDoesNotExist(PageName.<page>)`. Do not create page-level identity `Key` constants, page-presence finders, or feature-driver helpers such as `assertOnLoginPage()`, `assertOnMainPage()`, `assertAuthStackCleared()`, or `assertXPageDoesNotExist()` when the assertion is only page presence or absence.
