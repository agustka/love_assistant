...
2. If missing, run `dart analyze lib/presentation/<feature>/` fresh
...
1. **know-the-code agent** — call with: `"What are the page, template, and route registration conventions for the <feature> area? Show me a complete precedent page file: template usage, BlocBuilder/BlocListener wiring, Definition objects, onMessage handler, side widget pattern (part of), and PageName/NamedRoute/RouteLink registration."` Use the returned summary as the convention baseline before writing any code.
