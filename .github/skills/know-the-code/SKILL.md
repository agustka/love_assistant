...
name: know-the-code
description: >-
  Use this skill when investigating the isbapp codebase — before making changes,
  answering architecture questions, tracing feature flows, or deciding where new
  code should live. Trigger whenever an agent needs to navigate the codebase,
  find a precedent, understand a convention, or trace how data moves through
  layers. Prefer this over general Flutter or Clean Architecture advice.
---

# Know the Code

A procedural guide for investigating the `isbapp` Flutter codebase before answering questions or making changes. Use the code as the source of truth.

## Handoff I/O Contract

- Primary input: the current caller prompt/request.
- Do not use `.github/handoff/know-the-code.handoff.md` as implicit input for new investigations.
...
- Write `.github/handoff/know-the-code.handoff.md` only when the caller explicitly requests handoff output.
...
|---|---|---|
| Presentation | `lib/presentation/<feature>/` | page, template, organism, `BlocBuilder`/`BlocListener` wiring |
| Application | `lib/application/<feature>/` | Cubit, state, action methods |
| Domain | `lib/domain/<feature>/` | use case, repository interface, entity, value objects |
| Infrastructure | `lib/infrastructure/<feature>/` | repository impl, service, DTO/model, caching, API layer |
| Navigation/DI | route definitions, `RouteLink`, `NamedRoute`, `get_it` registrations | how the feature is wired and exposed |
...
Read a similar feature alongside the one you're investigating. If patterns differ, note which appears current or preferred.
...
- Cubits extend `IsbCubit`; states often extend `Equatable`
- Navigation uses `RouteLink` and `NamedRoute`
- Design system lives under `lib/presentation/core/isb/` (Atomic Design: atoms → molecules → organisms → templates → pages)
- Localization uses generated `S` accessors and ARB files

**Build environments:** `offline`, `test`, `production`

**Code generation**
Generated files exist, don't trace into them.
...
If caller requests `handoff` output, write `.github/handoff/know-the-code.handoff.md` using this structure:
