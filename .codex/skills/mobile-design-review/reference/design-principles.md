...
- **Inputs are headed cards, not bordered fields.** This app's input pattern is one input per `LaCardAtom` with a heading on top and the field inside (`LaTextField` `showCard: true`), cards sitting on the page background — see `LaWizardStepOrganism`. The card *is* the field boundary, so do not recommend adding a field outline/border for affordance, and never put an input card inside another card. When a form stacks several borderless fields inside one wrapping card, the fix is to restructure to one headed card per input on the background (heading, field, and that field's helper/strength/error inside its own card); form-level errors go on the background, not in a card.
