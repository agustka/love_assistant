...
description: Implements the infrastructure layer from .codex/specs/api.yaml. Generates models, services, repositories, caching, and streaming. Produces infrastructure.handoff.md.
---
...
- `.codex/specs/api.yaml` — the API contract (required)
- `.codex/handoff/domain.handoff.md` — if available, confirms domain entities exist (optional; determines whether steps 6–7 can proceed)
...
- .codex/handoff/infrastructure.handoff.md
...
Reference: see `lib/infrastructure/AGENTS.md` → "Offline Implementations" for the canonical annotations and folder rules.
...
2. **api-parsing** — extract endpoints, schemas, enums from .codex/specs/api.yaml
